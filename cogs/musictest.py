...
class Music2(commands.Cog, description="Jam to some awesome tunes! 🎶"):
...
**Channel:** [{song.channel}]({song.channel_url})
    ...
    async def lyrics(self, ctx: commands.Context, *, song=None):
        ...
        embeds = []
        async with self.bot.session.get(f'https://some-random-api.ml/lyrics?title={song.lower()}') as r:
            if r.status != 200:
