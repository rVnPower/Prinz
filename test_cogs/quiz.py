...
from utils.chat_format import box
...
class Quiz(commands.Cog):
    ...
    async def quizset_afk(self, ctx, questions: int):
        ...
        if 1 <= questions <= 10:
            await self.default_guild.afk.set(questions)
    ...
    async def quizset_show(self, ctx):
        ...
        show = self.default_guild.show_answer
        self.default_guild.show_answer.set(not show)
    ...
    async def quizset_questions(self, ctx, questions: int):
        ...
        if 5 <= questions <= 50:
            await self.default_guild.questions.set(questions)
    ...
    async def game(self, channel):
        ...
        for index, dictionary in enumerate(response["results"]):
            ...
            if len(user_answers) < 2:
                afk_questions += 1
                afk_count = self.default_guild.afk
            ...
            if self.default_guild.show_answer:
            ...
            questions = self.default_guild.questions
    ...
    async def end_game(self, channel):
        ...
        multiplier = self.default_guild.multiplier
        if multiplier == 0:
            self.playing_channels.pop(channel.id)
            return
        ...
        for _ in range(3):
            ...
            async with self.session.get("https://opentdb.com/api.php", params=parameters) as response:
                ...
                if response_code == 0:
                    ...
                    self.default_guild.token.set(None)
